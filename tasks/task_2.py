...
def save_speech_to_file(
    device: t_device, package: str, rate: int, speaker: str, text: str, curr_dir: Path,
) -> None:
...
async def asave_tts(lang: str, speaker: str, text: str) -> None:
    ...
    package_data = await get_speaker_package_data(silero_data, lang, speaker)
    ...
    with ProcessPoolExecutor() as pr_executor:
        loop.run_in_executor(
            pr_executor, save_speech_to_file, device, package, rate, speaker, text,
        )
